...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=PROJECT_ROOT / "reports" / "main_advanced" / "ragas.json")
    parser.add_argument("--details-output", type=Path, default=PROJECT_ROOT / "reports" / "main_advanced" / "ragas_details.jsonl")
    ...
    if args.limit is None and len(answers) != len(golden):
    ...
    with args.details_output.open(mode, encoding="utf-8") as handle:
        for start in range(0, len(pending), args.batch_size):
            ...
            result = evaluate(
                ...
                run_config=RunConfig(timeout=args.timeout, max_retries=args.max_retries, max_wait=10, max_workers=args.max_workers),
    ...
    # Retry mode appends updated rows while running for crash safety. Compact the
    # journal back to one final record per question after a successful pass.
