...
function App() {
  ...
  const updateEligibility = async (item, status) => { try { await api.updateEligibility(selected.document_id, item.id, status); const update = rows => (rows || []).map(row => row.id === item.id ? { ...row, user_status: status } : row); setData(prev => ({ ...prev, items: prev.items ? update(prev.items) : prev.items, eligibility: update(prev.eligibility), eligibility_items: update(prev.eligibility_items) })); setAnalysisCache(prev => { const next = { ...prev }; ["overview", "go"].forEach(suffix => { const key = `${selected.document_id}:${suffix}`; if (next[key]) next[key] = { ...next[key], eligibility: update(next[key].eligibility), eligibility_items: update(next[key].eligibility_items) }; }); return next; }); } catch (e) { setError(e.message); } };
...
function GoNoGoView({ data, onEvidence, onEligibility, onRisk }) {
  ...
  const review = riskSummary(riskItems, "review");
  const totalRiskClauses = riskItems.length;
  const eligibility = {
    total: eligibilityItems.length,
    notMet: eligibilityItems.filter(item => item.user_status === "not_met").length,
    reviewRequired: eligibilityItems.filter(item => item.user_status === "review_required").length,
    unchecked: eligibilityItems.filter(item => !item.user_status || item.user_status === "unchecked").length,
  };
  const eligibilityHeadline = eligibility.total > 0 ? `미충족 ${eligibility.notMet}건` : "분석 결과 없음";
  const additionalReviewCount = review.total
    + eligibility.reviewRequired
    + riskItems.filter(item => item.type !== "review" && item.user_status === "review_required").length;
  ...
  const reviewEvidence = riskItems.find(item => item.type === "review" && item.evidence)?.evidence;
  ...
  return <div className="goNoGoView">
    <div className="riskRatioSummary">
      <div className="eligibilitySummary" {...evidenceCardProps(eligibilityEvidence)}><small>참가 자격 대표 상태</small><strong>{eligibilityHeadline}</strong><span>전체 {eligibility.total}개 조항</span></div>
      <div className="disqualificationSummary" {...evidenceCardProps(disqualificationEvidence)}><small>실격 조항</small><strong>{dq.total}건</strong><span>위험 있음 {dq.atRisk} · 검토 전 {dq.unchecked}</span></div>
      <div className="deductionSummary" {...evidenceCardProps(deductionEvidence)}><small>감점·불이익 조항</small><strong>{dd.total}건</strong><span>위험 있음 {dd.atRisk} · 검토 전 {dd.unchecked}</span></div>
      <div className="reviewSummary" {...evidenceCardProps(reviewEvidence)}><small>추가 확인 조항</small><strong>{additionalReviewCount}건</strong><span>사용자 지정 {eligibility.reviewRequired + riskItems.filter(item => item.type !== "review" && item.user_status === "review_required").length}건 · 전체 위험 {totalRiskClauses}건</span></div>
    </div>
    <div className="sectionTitle">참가 자격</div>
    <div className="cards">{eligibilityItems.map(item => <div className="card" key={item.id}><button className="cardBody" onClick={() => onEvidence(item.evidence)}><strong>{item.title}</strong><p>{item.description}</p></button><div className="cardFooter"><EvidenceButton evidence={item.evidence} onEvidence={onEvidence}/><select value={item.user_status || "unchecked"} onChange={e => onEligibility(item, e.target.value)}><option value="unchecked">미확인</option><option value="met">충족</option><option value="not_met">미충족</option><option value="review_required">추가 확인 필요</option></select></div></div>)}</div>
...
function OverviewView({ data, onEvidence }) {
  ...
  const eligibilitySummary = eligibility.some(item => item.user_status === "not_met") ? "미충족" : eligibility.length > 0 && eligibility.every(item => item.user_status === "met") ? "충족" : "미확인";
