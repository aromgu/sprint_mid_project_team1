...
DEFAULT_INPUT_PATH = _SETTINGS.path("chunks")
DEFAULT_PERSIST_DIRECTORY = _SETTINGS.path("chroma")
DEFAULT_BM25_DIRECTORY = DEFAULT_PERSIST_DIRECTORY.with_name("bm25")
DEFAULT_REPORT_PATH = _SETTINGS.path("indexing_report")
DEFAULT_COLLECTION_NAME = str(_SETTINGS.get("index", "collection_name"))
DEFAULT_EMBEDDING_MODEL = str(_SETTINGS.get("index", "embedding_model"))
DEFAULT_BATCH_SIZE = int(_SETTINGS.get("index", "batch_size", 100))
EXPECTED_EMBEDDING_DIMENSION = int(
    _SETTINGS.get("index", "embedding_dimension", 1_536)
)
...
def audit_advanced_input(
    ...
    *,
    ...
    # The upstream repository approved one fixed 98-document corpus by SHA.
    # This port validates the same row-level contract, while also accepting a
    # newly generated workspace corpus whose hash is recorded in the report.
    contract = INPUT_CONTRACTS_BY_SHA256.get(input_sha256)
    ...
    for label, actual in (
        ...
        if len(actual) != 1 or not next(iter(actual), ""):
            raise ValueError(f"{label} 오류: {sorted(actual)}")

    if contract is not None and max_records is None:
        expected_values = {
    ...
    return AdvancedInputAudit(
        ...
        contract_name=contract.name if contract is not None else "workspace_validated",
