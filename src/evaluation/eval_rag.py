...
def _evaluate_hitk(retrievers: ProductionRetriever) -> pd.DataFrame:
    ...
    stages = {
        "Naive(Vector)": retrievers.retrieve_naive,
        "①Hybrid": retrievers.retrieve_hybrid,
        "②+Rerank": retrievers.retrieve_reranked,
    }
    rows = []
    for q, gold_id, typ in test_set:
        ...
        for name, fn in stages.items():
            row[name] = (
                "✅"
                if any(d.metadata.get("id") == gold_id for d in fn(q)[:3])
                else "❌"
            )
        rows.append(row)
...
def _run_ragas_metrics(
    advanced_chain: AdvancedRAGChain, config: RAGConfig
) -> pd.DataFrame:
    dataset_raw = get_ragas_testset()
    records = []
    for q, gt in zip(dataset_raw["questions"], dataset_raw["ground_truths"]):
        contexts = [
            d.page_content for d in advanced_chain.retrievers.retrieve_reranked(q)
        ]
        records.append(
            {
                "user_input": q,
    ...
    result = evaluate(
        ...
        llm=LangchainLLMWrapper(
            ChatOpenAI(model=config.llm_model, temperature=config.temperature)
        ),
...
def run_evaluation(
    advanced_chain: AdvancedRAGChain, retrievers: ProductionRetriever, config: RAGConfig
):
    df_hit = _evaluate_hitk(retrievers)
    df_hit.to_csv(
        os.path.join(config.report_dir, "hit_scoreboard.csv"),
        index=False,
        encoding="utf-8-sig",
    )
    df_ragas = _run_ragas_metrics(advanced_chain, config)
    df_ragas.to_csv(
        os.path.join(config.report_dir, "ragas_evaluation_result.csv"),
        index=False,
        encoding="utf-8-sig",
    )
